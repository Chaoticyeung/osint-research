...
(明确说不查什么,直接引用开工时写的两级调查纲目)、适用法域。

**1. 执行摘要 / BLUF**(Bottom Line Up Front,一页内):直接回答 KIQ、支持哪个假设、整体置信度
(校准语言)、最关键的 1–3 条依据。让不看正文的人也能拿去做判断。

**2. 范围与方法 + 覆盖声明(强制)**:查了什么、怎么查、用了哪些源并标优先级(**一手 vs 二手**、
公开/付费/登录墙)。读者关心发现,方法从简但要可复现。**覆盖声明拿调查纲目逐项对**:哪些已覆盖、
哪些只覆盖一部分、哪些完全没碰(原因:够不着/超预算/不合规/低价值),以及每个缺口让哪条结论变脆。
这里说的是"根本没查",与部件 10 的"查了但没结果"分开写。**再报一次来源位置覆盖**:当事方、
监管、对手方、第三方记录覆盖了哪几个,只有当事方自述时明说(见 methodology.md"覆盖缺口")。
不写覆盖声明,读者会默认你查全了——这是调查报告最容易造成的误导。
...
"名义人—直接关联人—接盘人—出资方"关系图。境外人物/公司的公开关系(任职、控股、亲属、制裁名单)可先用
Wikidata SPARQL(免注册)和 OpenSanctions(网页检索免注册,API 非商用免费申请 key)拉一版初稿再人工核对;两者只给"有公开记录的边",查不到不等于没关系。
...
- **摘录列只放逐字原文**:搜索摘要、抓取工具生成的页面概述、他人复述都不是摘录,它们会抹掉
  限定词、精确值和归属。仅得转述的事实降一级并注明来源,放"内部待核"(见 methodology.md
  "一手原文纪律")。台账另设"来源位置"列,用于收口前数视角覆盖。
...
有核实路径的线索进"内部调查日志"继续追(见 reading-signals.md)。**这里是"查了但没结果",
"根本没查"归部件 2 的覆盖声明,两者不要混写。**
